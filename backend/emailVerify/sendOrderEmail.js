...
const buildOrderHtml = (firstName, orderItems, orderTotal) => {
  const rows = orderItems
    ...
    <tr>
      ...
      <td style="padding: 8px; border: 1px solid #e5e7eb; text-align:center;">${item.qty ?? item.quantity}</td>
...
export const sendOrderEmail = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "sendOrderEmail: transporter.verify warning",
        v.message || v,
      );
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "sendOrderEmail: transporter2.verify warning",
          v2.message || v2,
        );
      }
      ...
      console.error(
        "sendOrderEmail: retry via IPv4 failed",
        err2.message || err2,
      );
      return false;
    }
